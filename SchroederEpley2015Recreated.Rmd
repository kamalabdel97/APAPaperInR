...
```{r, include=FALSE}
descriptives_df <- SmarterTalkerSpeaker %>% group_by(CONDITION) %>%
  summarize(Mean = mean(intellect),
   Median = median(intellect), 
   SD = sd(intellect)
   )

descriptives_df[, -1] <- printnum(descriptives_df[, -1])



# kable(descriptives_df, "latex", caption = "Mean Reaction Times and Standard Errors of the Mean for Experiment 3", booktabs = T) %>%
#   kable_styling() %>%
#   add_header_above(c(" " = 1, "Congruent" = 2, "Incongruent" = 2))
```

```{r, results="asis", eval=TRUE}
apa_table(descriptives_df,
